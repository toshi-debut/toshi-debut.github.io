...
//
//   【やり残し】Netlify の「Contains secret values」を ON に戻すこと。
//     ONにすると管理画面・API・ビルドログ・CLI のどこにも値が出なくなる。
//     Netlify 自身が推奨している設定だが、設定を切り分けるために今は OFF にしてある。
//     動作が安定したら、とくに本番キーを登録する前に、必ず ON を試すこと。
//     手順と判定方法は README の「シークレットキーの置き場所」に書いてある。
//
